...
public class GestureRecorder implements SensorEventListener {
	...
	public void onSensorChanged(SensorEvent sensorEvent) {
		...
		if (!PebbleBlehBluhBlah.events.isEmpty())
		{

			int[] temp = (int [])PebbleBlehBluhBlah.events.removeLast();
//			Log.e("sgs", Arrays.toString(temp));
			if (temp == null) return;

			value = new float[]{ (temp[0]/4000.0f*range), (temp[1]/4000.0f*range), (temp[2]/4000.0f*range)};

			count++;
//			Log.v("Saving", "" + count + " "  + Arrays.toString(value));
		...
		case MOTION_DETECTION:
			if (isRecording) {
				Log.v("recording", Arrays.toString(value));
